...
feature {NONE} -- Initialization
	...
	make (a_id: INTEGER_64; a_owner_id, a_folder_id: INTEGER_64;
		...
		do
			...
			metadata_json := if attached a_metadata_json as al_m then al_m.to_string_8 else "{}" end
			...
			expires_at := if attached a_expires_at as al_e then al_e.to_string_8 else Void end
			last_accessed_at := if attached a_last_accessed_at as al_l then al_l.to_string_8 else Void end
		end
...
feature -- Modification
	...
	set_expires_at (a_timestamp: detachable READABLE_STRING_8)
		do
			expires_at := if attached a_timestamp as al_t then al_t.to_string_8 else Void end
		end
